package wingtips.sampling;

import wingtips.Tracer;

/**
 * Pluggable strategy for {@link Tracer} that determines whether or not the next root span should be sampled (i.e. request span that has no parent and is not
 * explicitly requested to be sampled or ignored).
 * Call {@link Tracer#setRootSpanSamplingStrategy(RootSpanSamplingStrategy)} to tell the tracer to use a specific strategy.
 *
 * @author Nic Munroe
 */
