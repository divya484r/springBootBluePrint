...
import wingtips.lifecyclelistener.SpanLifecycleListener;
import wingtips.sampling.RootSpanSamplingStrategy;
import wingtips.sampling.SampleAllTheThingsStrategy;
import wingtips.util.TracerManagedSpanStatus;
 ...
 *     referred to as "sub-spans" in this class. Sub-spans are started via {@link #startSubSpan(String, Span.SpanPurpose)} and completed via {@link #completeSubSpan()}.
 ...
 *     <a href="https://github.com/sample-Inc/wingtips#async_usage">the asynchronous usage section of the Wingtips
 ...
 *     separate nested span with the overall-request-span as its parent you can do so via the "sub-span" methods: {@link #startSubSpan(String, Span.SpanPurpose)} and
 ...
 *     an error occurs. Each call to {@link #startSubSpan(String, Span.SpanPurpose)} causes the "current span" to be the new sub-span's parent, and causes the new sub-span to
 ...
 *
 *     <p>The <a href="https://github.com/sample-Inc/wingtips#async_usage">asynchronous usage section of the Wingtips
...
public class Tracer {
     ...
     * <p/>
     * NOTE: If {@link #currentSpanStackThreadLocal} is null or empty for this thread it will try to reconstitute the {@link Span} from the logging {@link MDC}.
     ...
     * {@link #startRequestWithSpanInfo(String, String, String, boolean, String, Span.SpanPurpose)} instead). The newly created root span will have a
     ...
     * {@link #startSubSpan(String, Span.SpanPurpose)} instead.
     *
     ...
     * {@link #startRequestWithChildSpan(Span, String)} or {@link #startRequestWithSpanInfo(String, String, String, boolean, String, Span.SpanPurpose)} instead).
     ...
     * @return The new span (which is now also the current one that will be returned by {@link #getCurrentSpan()}).
     */
    public Span startRequestWithSpanInfo(String traceId, String parentSpanId, String newSpanName, boolean sampleable, String userId, Span.SpanPurpose spanPurpose) {
     ...
     */
    public Span startSubSpan(String spanName, Span.SpanPurpose spanPurpose) {
     ...
     * SpanPurpose#CLIENT}) then use the {@link #startSpanInCurrentContext(String, Span.SpanPurpose)} method instead.
     *
     ...
     * javadoc - abusing this method can lead to broken tracing without any errors showing up in the logs.
     */
    public Span startSpanInCurrentContext(String spanName, Span.SpanPurpose spanPurpose) {
     ...
     * @return The new span (which is now also the current one that will be returned by {@link #getCurrentSpan()}).
     */
    protected Span doNewRequestSpan(String traceId, String parentSpanId, String newSpanName, boolean sampleable, String userId, Span.SpanPurpose spanPurpose) {
        if (newSpanName == null)
     ...
     * {@link MDC} so it is available there.
     */
    ...
    /**
     * Adds the given listener to the {@link #spanLifecycleListeners} list using {@link List#add(Object)}. This method will do nothing if you pass in null.
     * <p/>
    ...
    /**
     * Returns the value of calling {@link List#remove(Object)} on {@link #spanLifecycleListeners}.
     */
     ...
     * processed.
     * <br/>
     * When processing switches back to the original request, you can call {@link #registerWithThread(Deque)} to set the thread's span stack and MDC info back to the
